...
__attribute__((naked)) void closure_bridge_template() {
    ...
    xASM("mov x0, sp");
    xASM("mov x1, x14");
    xASM("bl _interceptor_routing_common_bridge_handler");
...
__attribute__((naked)) void dynamic_closure_bridge_template() {
    // DO NOT USE prologue
    // x29 == fp, x30 == lr
    // xASM("stp x29, x30, [sp, #-16]!");
    // xASM("mov x29, sp");
    ...
    xASM("mov x0, sp");
    xASM("mov x1, x14");
    xASM("bl _interceptor_routing_dynamic_common_bridge_handler");
...
// void closure_bridge_objc_msgSend() {
//     // save {q0-q7}
//     xASM("sub sp, sp, #(8*16)");
//     xASM("stp q6, q7, [sp, #(6*16)]");
//     xASM("stp q4, q5, [sp, #(4*16)]");
